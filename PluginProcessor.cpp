...
#endif
{
    ...
    floatHelper(inputGainParm, Names::Gain_In);
    floatHelper(outputGainParm, Names::Gain_Out);
...
void CompressorAudioProcessor::updateState()
{
    ...
    auto midHiighCutoffFreq = midHighCrossover->get();
    AP2.setCutoffFrequency(midHiighCutoffFreq);
    LP2.setCutoffFrequency(midHiighCutoffFreq);
    HP2.setCutoffFrequency(midHiighCutoffFreq);

    inputGain.setGainDecibels(inputGainParm->get());
    outputGain.setGainDecibels(outputGainParm->get());

}

void CompressorAudioProcessor::splitBands(const juce::AudioBuffer<float>& inputBuffer)
{
